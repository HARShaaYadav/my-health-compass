...
export default function ConsultationPage() {
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["consultations"] }); toast.success("Appointment cancelled"); },
  });
    ...
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      {bookings.filter((b) => b.status === "booked").length > 0 && (
        <div className="space-y-3">
          ...
          {bookings.filter((b) => b.status === "booked").map((b) => (
            <motion.div key={b._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="clinical-card-info flex items-center justify-between">
              <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => cancelMutation.mutate(b._id)} className="text-xs text-muted-foreground hover:text-destructive">Cancel</button>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <div className="clinical-card !p-3">
        <div className="flex gap-2">
          ...
          <Input placeholder="Search by specialty, doctor name..." className="border-0 shadow-none focus-visible:ring-0" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      </div>

      <div className="space-y-3">
        {filteredDoctors.map((doc, i) => (
          <motion.div key={doc.name} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="clinical-card">
                ...
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                  <div className="mt-4 pt-4 border-t border-border space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => setBookingDoctor(null)} className="p-1 text-muted-foreground hover:text-foreground"><X className="h-4 w-4" /></button>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {doc.slots.map((slot) => (
                        <button key={slot} onClick={() => setSelectedSlot(slot)} className={`text-xs px-3 py-2 rounded-lg border transition-colors ${selectedSlot === slot ? "bg-primary/10 border-primary/30 text-primary font-medium" : "border-border text-muted-foreground hover:border-primary/30"}`}>
                          <Calendar className="h-3 w-3 inline mr-1.5" />{slot}
                        </button>
                      ))}
                    </div>
                    <Button className="w-full gap-2" disabled={!selectedSlot || bookMutation.isPending} onClick={() => bookMutation.mutate()}>
                      {bookMutation.isPending ? <span className="h-4 w-4 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" /> : <><Check className="h-4 w-4" />Confirm Booking — {doc.fee}</>}
                    </Button>
                  </div>
                </motion.div>
              )}
